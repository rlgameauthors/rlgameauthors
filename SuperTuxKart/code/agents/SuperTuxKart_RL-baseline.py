...
class Agent:
    ...
    def play_step(self, net, epsilon=0.0, device="cpu"):
        ...
        if is_done or elapsed_time > GAME_LENGTH:
            ...
            print('score %2f, last path_done %2f,  game time: %2f, game completed %d ... time %s' % (
                done_reward, self.path_done, info['game_time'], self.count_completed, datetime.now()))
...
if __name__ == "__main__":
    ...
    while True:
        ...
        reward_game, time_consumption_, game_actions = agent.play_step(net, epsilon, device=device)
        if reward_game is not None:
            ...
            if iteration % 100 == 0:
                # save actions
                f_actions = open('games_actions_RL-baselineE.txt', 'w')
                ...
                time_consumption_games_df.to_csv('fps_info_RL-baseline.csv')
